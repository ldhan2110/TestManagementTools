...
const isSeniorTesterMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    //{throw new HttpException(400, 'Require senior tester role');}
      return res.status(400).json({status: 400, success:false, result:'', errMsg: 'Require senior tester role'});
  } catch (error) {
    res.status(400).json({status: 400, success:false, result:'', errMsg: 'Role is not valid'});
  }
};

export default isSeniorTesterMiddleware;