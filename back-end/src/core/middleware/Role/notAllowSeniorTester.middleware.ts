...
const notAllowSeniorTesterMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    (    
      ...
      project.members.some((item: IMember) =>
        ...
        item.role.toString() !== 'seniortester'

      )
    )next();
    else
    //{throw new HttpException(400, 'Require senior tester role');}
      return res.status(400).json({status: 400, success:false, result:'', errMsg: 'Do not allow senior tester role'});
  } catch (error) {
    res.status(400).json({status: 400, success:false, result:'', errMsg: 'Role is not valid'});
  }
};

export default notAllowSeniorTesterMiddleware;