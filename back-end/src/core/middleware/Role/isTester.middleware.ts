...
const isTesterMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    //{throw new HttpException(400, 'Require tester role');}
      return res.status(400).json({status: 400, success:false, result:'', errMsg: 'Require Tester role'});
  } catch (error) {
    res.status(400).json({status: 400, success:false, result:'', errMsg: 'Role is not valid'});
  }
};

export default isTesterMiddleware;